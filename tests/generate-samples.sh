...
$FFMPEG -y -f lavfi -i "testsrc=duration=1:size=320x240:rate=30" \
    ...
    "$SAMPLES_DIR/test_raw.yuv" 2>/dev/null
echo "Created: test_raw.yuv"

# PNG image
$FFMPEG -y -f lavfi -i "testsrc=duration=0.1:size=320x240:rate=1" \
    -frames:v 1 -c:v png \
    "$SAMPLES_DIR/test.png" 2>/dev/null
echo "Created: test.png"

# MJPEG
$FFMPEG -y -f lavfi -i "testsrc=duration=1:size=320x240:rate=30" \
    -c:v mjpeg -q:v 5 \
    "$SAMPLES_DIR/test_mjpeg.avi" 2>/dev/null
echo "Created: test_mjpeg.avi"
...
$FFMPEG -y -f lavfi -i "sine=frequency=440:duration=1" \
    ...
    "$SAMPLES_DIR/test_pcm_s16le.wav" 2>/dev/null
echo "Created: test_pcm_s16le.wav"

# Raw PCM (s24le)
$FFMPEG -y -f lavfi -i "sine=frequency=440:duration=1" \
    -c:a pcm_s24le -ar 44100 \
    "$SAMPLES_DIR/test_pcm_s24le.wav" 2>/dev/null
echo "Created: test_pcm_s24le.wav"
...
$FFMPEG -y -f lavfi -i "testsrc=duration=1:size=320x240:rate=30" \
    ...
    "$SAMPLES_DIR/test.mov" 2>/dev/null
echo "Created: test.mov"

# Matroska/MKV
$FFMPEG -y -f lavfi -i "testsrc=duration=1:size=320x240:rate=30" \
    -f lavfi -i "sine=frequency=440:duration=1" \
    -c:v mjpeg -q:v 5 -c:a pcm_s16le \
    "$SAMPLES_DIR/test.mkv" 2>/dev/null
echo "Created: test.mkv"
...
$FFMPEG -y -f lavfi -i "testsrc=duration=1:size=320x240:rate=30" \
    -f lavfi -i "sine=frequency=440:duration=1" \
    -c:v mjpeg -q:v 5 -c:a pcm_s16le \
    "$SAMPLES_DIR/test_mux.mkv" 2>/dev/null
echo "Created: test_mux.mkv (for muxer testing)"

# AVI
$FFMPEG -y -f lavfi -i "testsrc=duration=1:size=320x240:rate=30" \
    -f lavfi -i "sine=frequency=440:duration=1" \
    -c:v mjpeg -q:v 5 -c:a pcm_s16le \
    "$SAMPLES_DIR/test.avi" 2>/dev/null
echo "Created: test.avi"

# Ogg with PCM (will use FLAC since Ogg supports it)
$FFMPEG -y -f lavfi -i "sine=frequency=440:duration=1" \
    -c:a flac \
    "$SAMPLES_DIR/test.ogg" 2>/dev/null
echo "Created: test.ogg"

# FLAC
$FFMPEG -y -f lavfi -i "sine=frequency=440:duration=1" \
    -c:a flac \
    "$SAMPLES_DIR/test.flac" 2>/dev/null
echo "Created: test.flac"

# WAV
$FFMPEG -y -f lavfi -i "sine=frequency=440:duration=1" \
    -c:a pcm_s16le \
    "$SAMPLES_DIR/test.wav" 2>/dev/null
echo "Created: test.wav"

# Image sequence
mkdir -p "$SAMPLES_DIR/image_seq"
$FFMPEG -y -f lavfi -i "testsrc=duration=0.5:size=320x240:rate=10" \
    -c:v png \
    "$SAMPLES_DIR/image_seq/frame_%03d.png" 2>/dev/null
echo "Created: image_seq/frame_*.png"
